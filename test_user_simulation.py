...
def rate_search_results(
    results: List[dict], query: str, expected_topics: List[str]
) -> Dict[str, Any]:
    ...
    return {
        ...
        "has_descriptions": round(has_descriptions / len(results) * 100, 1)
        if results
        else 0,
    }
...
def test_intent_extraction(query_data: dict) -> Dict[str, Any]:
    ...
    if not success:
        return {"success": False, "latency_ms": latency, "error": response}
    ...
    goal_match = (
        goal.lower() == query_data["expected_intent"].lower() if goal else False
    )
...
def test_search(query_data: dict) -> Dict[str, Any]:
    ...
    quality_metrics = rate_search_results(
        results, query_data["query"], query_data["expected_topics"]
    )
...
def simulate_user_session(user_id: str, queries: List[dict]) -> Dict[str, Any]:
    ...
    for i, query_data in enumerate(queries, 1):
        print(
            f"{Colors.BOLD}Query {i}/{len(queries)}: {query_data['category']}{Colors.ENDC}"
        )
        ...
        if intent_result["success"]:
            print_success(
                f"{intent_result['latency_ms']}ms - Goal: {intent_result['goal']}"
            )
        else:
        ...
        if search_result["success"]:
            ...
            print_success(
                f"{search_result['latency_ms']}ms - {search_result['result_count']} results - Quality: {quality['relevance_score']}/10"
            )
        else:
            print_error(f"Failed: {search_result.get('error', {})}")

        results.append(
            {"query": query_data, "intent": intent_result, "search": search_result}
        )
        print()
    ...
    intent_latencies = [
        r["intent"]["latency_ms"] for r in results if r["intent"]["success"]
    ]
    search_latencies = [
        r["search"]["latency_ms"] for r in results if r["search"]["success"]
    ]

    quality_scores = [
        r["search"]["quality"]["relevance_score"]
        for r in results
        if r["search"]["success"]
    ]
    ...
    return {
        ...
        "intent_accuracy": round(successful_intents / len(queries) * 100, 1)
        if queries
        else 0,
        "search_success_rate": round(successful_searches / len(queries) * 100, 1)
        if queries
        else 0,
        "avg_intent_latency_ms": round(statistics.mean(intent_latencies), 2)
        if intent_latencies
        else 0,
        "avg_search_latency_ms": round(statistics.mean(search_latencies), 2)
        if search_latencies
        else 0,
        "p95_intent_latency_ms": round(
            statistics.quantiles(intent_latencies, n=100)[94], 2
        )
        if len(intent_latencies) > 1
        else 0,
        "p95_search_latency_ms": round(
            statistics.quantiles(search_latencies, n=100)[94], 2
        )
        if len(search_latencies) > 1
        else 0,
        ...
        for result in session["results"]:
            ...
            if result["search"]["success"]:
                ...
                all_quality_scores.append(
                    result["search"]["quality"]["relevance_score"]
                )
...
"""
    ...
    if statistics.mean(all_search_latencies) > 5000:
        issues.append(
            "⚠️ **High Search Latency**: Average >5s (likely due to SearXNG API calls)"
        )
...
def main():
    ...
    print(
        f"Avg Search Latency: {avg_search_latency:.2f}ms ({avg_search_latency / 1000:.1f}s)"
    )
