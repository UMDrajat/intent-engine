...
# Use in-memory database for testing to ensure a clean state and avoid conflicts
...
def test_complete_workflow():
    # Use TestClient as a context manager to trigger lifespan events (like DB creation)
    with TestClient(app) as client:
        # Local mock for requests to minimize changes in the function body
        class requests_mock:
            @staticmethod
            def post(url, **kwargs):
                return client.post(url.replace(BASE_URL, ""), **kwargs)

            @staticmethod
            def get(url, **kwargs):
                return client.get(url.replace(BASE_URL, ""), **kwargs)
        ...
        r = requests.post(
            ...
            json={"name": advertiser_name, "contact_email": "workflow@test.com"},
        )
        ...
        r = requests.post(
            f"{BASE_URL}/conversion-tracking",
            json={
                "click_id": click_id,
        ...
        if r.status_code in [
            200,
            404,
        ]:  # 404 is acceptable if campaign exists but metrics don't
            results["passed"] += 1
        ...
        if r.status_code == 200:
            ...
            r = requests.post(
                f"{BASE_URL}/ab-tests/{test_id}/variants",
                json={
                    "test_id": test_id,
                    "name": "control",
                    "ad_id": ad_id,
            ...
            if r.status_code == 200:
                ...
                results["details"].append(
                    ("Add control variant", r.status_code, r.text)
                )
            ...
            if r.status_code == 200:
                ...
                results["failed"] += 1
                results["details"].append(
                    ("Add treatment variant", r.status_code, r.text)
                )
            ...
            # 10.5 Assign User
            r = requests.post(
                f"{BASE_URL}/ab-tests/{test_id}/assign",
                params={"user_identifier": "test-user-123"},
            )
            if r.status_code == 200:
                results["passed"] += 1
                assignment = r.json()
                print(
                    f"    10.5 SUCCESS: Assigned user to variant: {assignment.get('variant_name')}"
                )
            else:
                results["failed"] += 1
            ...
            if r.status_code == 200:
                ...
                results["failed"] += 1
                results["details"].append(
                    ("Get A/B test results", r.status_code, r.text)
                )
        ...
        print("\n11. Verifying persistence (Listing campaigns)...")
        r = requests.get(
            f"{BASE_URL}/campaigns", params={"advertiser_id": advertiser_id}
        )
        if r.status_code == 200:
            campaigns = r.json()
            if any(c["id"] == campaign_id for c in campaigns):
                ...
                results["failed"] += 1
                results["details"].append(
                    ("Verify campaign persistence", 200, "Campaign not in list")
                )
