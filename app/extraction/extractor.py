...
class IntentExtractor:
    ...
    def extract_intent_from_request(
        ...
        session_id = (
            getattr(request.context, "sessionId", None) or f"sess_{uuid.uuid4().hex}"
        )
        ...
        is_programming, prog_confidence = programming_extractor.is_programming_query(
            text
        )
        ...
        if is_programming:
            logger.info(
                f"Programming query detected (confidence: {prog_confidence:.2f})"
            )
            programming_context = programming_extractor.extract_programming_context(
                text
            )
            logger.info(
        ...
        if is_programming and programming_context:
            programming_use_cases = programming_extractor.get_use_cases(
                programming_context
            )
