...
from app.ranking.ranker import EmbeddingCache
    ...
    async def rank_urls(self, request: URLRankingRequest) -> URLRankingResponse:
        ...
        if request.titles or request.contents:
            for i, result in enumerate(analyzed_urls):
                ...
                if (
                    request.contents
                    and i < len(request.contents)
                    and request.contents[i]
                ):
        ...
        for analysis in analysis_results:
            results.append(
                URLResult(
                    ...
                    encryption_enabled=analysis.quality_indicators.get(
                        "encryption", True
                    ),
                    content_type=analysis.content_type,
                    is_open_source=analysis.quality_indicators.get(
                        "is_open_source", False
                    ),
                    is_non_profit=analysis.quality_indicators.get(
                        "is_non_profit", False
                    ),
                    quality_score=0.5,
