...
class SearchResult:
    ...
    def __post_init__(self):
        ...
        if self.url and not self.domain:
            try:
                self.domain = self.url.split("/")[2].lower() if "/" in self.url else self.url.lower()
            except:
                self.domain = ""
...
class EnhancedRanker:
    ...
    def _generate_cache_key(self, candidates: list[dict], intent: UniversalIntent) -> str:
    ...
    async def _cache_ranking_results(self, cache_key: str, results: list[RankedResult], ttl: int = 300):
    ...
    async def rank_with_filters(
        ...
        logger.info(f"Ranking cache MISS, computing...")
        ...
        logger.debug(f"Filtered {len(search_results)} → {len(filtered_results)} results")
        ...
        asyncio.create_task(self._cache_ranking_results(cache_key, scored_results, ttl=300))
        
        elapsed = (time.time() - start_time) * 1000
        logger.info(f"Enhanced ranking complete: {len(scored_results)} results in {elapsed:.2f}ms")
    ...
    def _convert_to_search_results(self, candidates: list[dict]) -> list[SearchResult]:
        ...
        for candidate in candidates:
            try:
                result = SearchResult(
                    id=candidate.get("id", hashlib.md5(candidate.get("url", "").encode()).hexdigest()),
    ...
    def _apply_content_filters(
        ...
        for result in results:
            ...
            if result.qualityScore and result.qualityScore < self.filter_config.get("min_quality_threshold", 0.3):
    ...
    def _compute_semantic_similarity(self, result: SearchResult, intent: UniversalIntent) -> float:
    ...
    def _compute_content_quality(self, result: SearchResult) -> float:
        ...
        if QUALITY_THRESHOLDS["min_title_length"] <= title_len <= QUALITY_THRESHOLDS["max_title_length"]:
