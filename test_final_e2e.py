...
def main():
    ...
    success, data, latency = test_endpoint("Intent Extraction", "/extract-intent", "POST", {
        "product": "search",
        "input": {"text": "best python tutorial"},
        "context": {"sessionId": "test"}
    })
    goal = data.get('intent', {}).get('declared', {}).get('goal', 'unknown') if success else 'N/A'
    ...
    success, data, latency = test_endpoint("Search", "/search", "POST", {
    ...
    print(f"   {'✓' if success else '✗'} {latency/1000:.2f}s - {result_count} results (API: {processing_time:.2f}s)")
