...
class BenchmarkStats:
    ...
    def add_result(self, response_time_ms: float, success: bool):
        ...
        self.p95_time_ms = sorted_times[p95_idx] if p95_idx < len(sorted_times) else sorted_times[-1]
        self.p99_time_ms = sorted_times[p99_idx] if p99_idx < len(sorted_times) else sorted_times[-1]
...
class APITester:
    ...
    def test_health_endpoints(self):
        ...
        for method, endpoint, description in endpoints:
            ...
            print(f"  {status} Status: {result.status_code}, Time: {result.response_time_ms:.2f}ms")
    ...
    def test_intent_extraction(self):
        ...
        for query in test_queries:
            ...
            if result.success:
                try:
                    data = result.json() if hasattr(result, 'json') else json.loads(requests.post(f"{self.base_url}/extract-intent", json=payload).text)
    ...
    def test_search_endpoints(self):
        ...
        test_queries = [
            {"query": "best laptop for programming", "extract_intent": True, "rank_results": True, "max_results": 5},
            {"query": "python tutorials for beginners", "extract_intent": True, "rank_results": False, "max_results": 5},
            {"query": "compare electric cars 2026", "extract_intent": True, "rank_results": True, "max_results": 5},
            {"query": "how to fix memory leak in python", "extract_intent": True, "rank_results": True, "max_results": 5},
            {"query": "best budget smartphones", "extract_intent": False, "rank_results": False, "max_results": 5},
        ]
        ...
        for payload in test_queries:
            ...
            status = "✓" if result.success else "✗"
            print(f"  {status} Status: {result.status_code}, Time: {result.response_time_ms:.2f}ms, Results: {result.result_count}")
            
            # Check for zero results
            if result.success and result.result_count == 0:
                print(f"  ⚠ WARNING: Zero results returned!")
                self.issues.append({
                    "type": "zero_results",
                    "endpoint": "/search",
                    "query": payload['query'],
                ...
                print(f"  ⚠ Note: Only {result.result_count} results (expected {payload.get('max_results', 5)})")
    ...
    def test_ranking_endpoints(self):
        ...
        payload = {
            ...
            "results": [
                {"title": "Python for Beginners", "url": "https://python.org/beginners", "content": "Learn Python basics"},
                {"title": "Advanced Python", "url": "https://realpython.com", "content": "Advanced Python techniques"},
                {"title": "Python Data Science", "url": "https://pandas.pydata.org", "content": "Python for data science"},
            ],
            "intent": {
                "declared": {
                    "query": "best python tutorials",
    ...
    def test_service_recommendation(self):
        ...
        payload = {
            ...
            "available_services": [
                {"id": "tutorial-service", "type": "EDUCATIONAL", "capabilities": ["LEARNING", "TUTORIALS"]},
                {"id": "search-service", "type": "SEARCH", "capabilities": ["INFORMATION_RETRIEVAL"]},
                {"id": "forum-service", "type": "COMMUNITY", "capabilities": ["Q&A", "DISCUSSION"]}
            ]
        }
    ...
    def test_ad_matching(self):
        ...
        status = "✓" if result.success else "✗"
        print(f"  {status} Status: {result.status_code}, Time: {result.response_time_ms:.2f}ms, Ads: {result.result_count}")
    ...
    def run_benchmark(self, endpoint: str, method: str = "POST", payload: dict = None, iterations: int = 10):
        ...
        for i in range(iterations):
            ...
            if self.verbose:
                status = "✓" if result.success else "✗"
                print(f"  [{i+1}/{iterations}] {status} {result.response_time_ms:.2f}ms")
    ...
    def print_summary(self):
        ...
        print(f"Total Time: {sum(r.response_time_ms for r in self.results)/1000:.2f}s")
        ...
        if table_data:
            print(tabulate(
                table_data,
                headers=["Endpoint", "Requests", "Success Rate", "Avg", "Min", "Max", "P95"],
                tablefmt="grid"
            ))
        ...
        for issue_type, issues in issues_by_type.items():
            ...
            for issue in issues:
                ...
                if issue.get('type') == 'zero_results':
                    print(f"    Payload: {json.dumps(issue.get('payload', {}), indent=6)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Intent Engine API Test & Benchmark Suite")
