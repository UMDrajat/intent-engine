...
def test_intent_extraction() -> List[Tuple[str, bool, float]]:
    ...
    for name, query in test_cases:
        print(f"Testing {name}...", end=" ")
        success, response, latency = make_request("/extract-intent", "POST", {
            "product": "search",
            "input": {"text": query},
...
def test_search() -> List[Tuple[str, bool, float]]:
    ...
    test_cases = [
        ...
        ("With Intent", {"query": "best programming laptop", "extract_intent": True, "max_results": 5}),
        ("With Ranking", {"query": "python web frameworks", "rank_results": True, "max_results": 5}),
    ]
    ...
    for name, data in test_cases:
        ...
        if success:
            ...
            print_success(f"{latency/1000:.2f}s - {result_count} results (API: {processing_time:.2f}s)")
            results.append((name, True, latency))
        else:
            print_error(f"{latency/1000:.2f}s - {str(response.get('error', 'Failed'))[:50]}")
            results.append((name, False, latency))
...
def test_ranking() -> List[Tuple[str, bool, float]]:
    ...
    success, response, latency = make_request("/rank-results", "POST", {
        "query": "python tutorials",
        "candidates": [
            {"url": "https://python.org", "title": "Python Official", "content": "Python programming language"},
            {"url": "https://realpython.com", "title": "Real Python", "content": "Python tutorials"}
        ],
        "intent": {
...
def test_service_recommendation() -> List[Tuple[str, bool, float]]:
    ...
    success, response, latency = make_request("/recommend-services", "POST", {
        "intent": {
...
def test_ad_matching() -> List[Tuple[str, bool, float]]:
    ...
    success, response, latency = make_request("/match-ads", "POST", {
        "intent": {
            "declared": {
                "goal": "compare",
                "constraints": [],
    ...
    if success:
        ...
        print_error(f"{latency/1000:.2f}s - {str(response.get('detail', 'Failed'))[:50]}")
...
def test_campaign_management() -> List[Tuple[str, bool, float]]:
    ...
    success, response, latency = make_request("/advertisers", "POST", {
...
def generate_final_report(all_results: Dict[str, List[Tuple[str, bool, float]]]):
    ...
    print(f"{'Category':<30} {'Tests':<8} {'Passed':<8} {'Failed':<8} {'Success':<10} {'Avg Latency':<12}")
    print("-" * 84)
    ...
    for category, results in all_results.items():
        ...
        avg_latency = sum(lat for _, success, lat in results if success) / max(passed, 1)
        ...
        print(f"{category:<30} {tests:<8} {passed:<8} {failed:<8} {success_rate:>5.0f}%      {avg_latency/1000:>6.2f}s")
    
    print("-" * 84)
    overall_success = (total_passed / total_tests * 100) if total_tests > 0 else 0
    overall_latency = total_latency / max(total_passed, 1)
    print(f"{'TOTAL':<30} {total_tests:<8} {total_passed:<8} {total_tests - total_passed:<8} {overall_success:>5.0f}%      {overall_latency/1000:>6.2f}s")
    print()
