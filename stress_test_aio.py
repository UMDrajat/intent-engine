...
class StressTestResults:
    ...
    def summary(self):
        ...
        return {
            ...
            "p95_response_time": f"{sorted(self.response_times)[int(len(self.response_times) * 0.95)]:.2f}ms"
...
def make_request(method, endpoint, data=None, headers=None):
    ...
    try:
        if method == "GET":
            resp = requests.get(
                f"{BASE_URL}{endpoint}", timeout=TIMEOUT, headers=headers
            )
        elif method == "POST":
            resp = requests.post(
                f"{BASE_URL}{endpoint}", json=data, timeout=TIMEOUT, headers=headers
            )
        elif method == "PUT":
            resp = requests.put(
                f"{BASE_URL}{endpoint}", json=data, timeout=TIMEOUT, headers=headers
...
def test_intent_extraction():
    ...
    for query in queries:
        ...
        success, rt, status, error = make_request(
            "POST", "/extract-intent", data=payload
        )
...
def test_search_api():
    ...
    for query in search_queries:
        payload = {"query": query, "limit": 10, "intent": {"goal": "LEARN"}}
...
def test_concurrent_requests(num_requests=50, num_workers=10):
    ...
    def worker(request_id):
        payload = {
            ...
            "context": {
                "sessionId": f"concurrent-test-{request_id}",
                "userLocale": "en-US",
            },
        }
        success, rt, status, error = make_request(
...
def test_edge_cases():
    ...
    test_cases = [
        ...
        (
            "Very long query",
            {"product": "search", "input": {"text": "a" * 10000}, "context": {}},
        ),
        ("Missing fields", {"product": "search"}),
        ("Invalid JSON type", {"product": 123, "input": "not a dict"}),
        (
            "Unicode query",
            {"product": "search", "input": {"text": "你好世界"}, "context": {}},
        ),
        (
    ...
    for name, payload in test_cases:
        ...
        results.add_result(success, rt, status, error)
        status_str = (
            "✓" if 200 <= (status or 0) < 500 else "✗"
        )  # Any non-500 is acceptable for edge cases
        print(f"  {status_str} {name}: {status} ({rt:.1f}ms)")
        if error and "Timeout" in str(error):
            print("      ⚠ TIMEOUT - This is a potential issue!")
...
def test_rate_limiting():
    ...
    for i in range(150):
        payload = {
            "product": "search",
            "input": {"text": f"rate limit test {i}"},
            "context": {},
        }
        success, rt, status, error = make_request(
            "POST", "/extract-intent", data=payload
...
def run_all_tests():
    ...
    total_requests = sum(
        ...
        if isinstance(r, StressTestResults)
    )
    total_success = sum(
